...
void ofApp::update(){
    if(serial.available()){
       receiveString = ofxGetSerialString(serial, '\n'); //read until end of line
        if(!receiveString.empty())v=ofToInt(receiveString); //check if string is not empty because it's sometimes empty
    }
}
...
void ofApp::draw(){
    ...
    if(v>50)ofBackground(255);
    else  ofBackground(0);
   
    string msg;
	msg += "receive: " + ofToString(v)+ "\n";
